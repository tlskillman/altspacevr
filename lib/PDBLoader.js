...
THREE.PDBLoader.prototype = {
	...
	parsePDB: function ( text ) {
		...
		for ( var i = 0, il = lines.length; i < il; ++ i ) {
			...
			if ( lines[ i ].substr( 0, 5 ) == "MODEL" ) {
				modelIndex = parseInt( lines[ i ].substr( 12, 4 ) );
			}
			else if ( !modelIndex ) {
				continue;
			}
			else if( lines [ i ].substr( 0, 6 ) == "ENDMDL" ) {
				modelIndex = false;
			}
			else if ( lines[ i ].substr( 0, 4 ) == "ATOM" || lines[ i ].substr( 0, 6 ) == "HETATM" ) {
				...
				console.log('bond line detected');
